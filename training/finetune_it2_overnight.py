#!/usr/bin/env python3
"""
Big high-quality IndicTrans2 LoRA fine-tune (commercial MIT path).

Uses training/data/train_big_quality.jsonl (± roman). Saves adapters only
(never merge_and_unload) under:
  training/artifacts/it2_big_quality_en_ne_lora/
  training/artifacts/it2_big_quality_ne_en_lora/
"""
...
TRAIN = DATA / "train_overnight_hybrid.jsonl"
VAL = DATA / "val_overnight_hybrid.jsonl"
ROMAN = DATA / "train_overnight_roman.jsonl"
EN_INDIC = REPO / "training" / "artifacts" / "it2_en_indic_merged"
INDIC_EN = REPO / "training" / "artifacts" / "it2_indic_en_merged"
OUT_EN_NE = REPO / "training" / "artifacts" / "it2_overnight_hybrid_en_ne_lora"
OUT_NE_EN = REPO / "training" / "artifacts" / "it2_overnight_hybrid_ne_en_lora"
...
def prefix_en(en: str, formality: str, use_formality: bool) -> str:
    if not use_formality:
        return en
    if formality == "formal":
        return f"<formal> {en}"
...
def make_dataset(
    ...
    if direction == "en-ne":
        ...
        for r in rows:
            src_texts.append(
                prefix_en(r["eng_Latn"], r.get("formality", "neutral"), use_formality)
            )
...
def train_one(
    ...
    print(f"[it2-overnight] === {direction} n_train={len(train_rows)} → {out_dir} ===", flush=True)
    ...
    if not found:
        found = ["q_proj", "v_proj", "k_proj", "fc1", "fc2"]
    print(f"[it2-overnight] LoRA targets={found}", flush=True)
    ...
    train_ds = make_dataset(
        train_rows, direction, ip, tok, args.max_length, args.use_formality_prefix
    )
    val_ds = make_dataset(
        val_rows, direction, ip, tok, args.max_length, args.use_formality_prefix
    )
    ...
    meta = {
        ...
        "architecture": "overnight_hybrid_quality_first",
        "formality_prefixes": args.use_formality_prefix,
        "lora_r": args.lora_r,
        "epochs": args.epochs,
    ...
    print(f"[it2-overnight] saved {adapter_dir}", flush=True)
...
def main() -> int:
    ...
    ap.add_argument("--max-roman", type=int, default=6_000)
    ...
    ap.add_argument(
        "--use-formality-prefix",
        action="store_true",
        help="OFF by default tonight — train meaning/fluency; register at inference",
    )
    args = ap.parse_args()
    ...
    if not TRAIN.exists():
        print("Run training/prepare_overnight_hybrid.py first", flush=True)
        return 1

    train_rows = load_jsonl(TRAIN, args.max_train)
    val_rows = load_jsonl(VAL, args.max_val)
    roman = load_jsonl(ROMAN, args.max_roman)
    ...
    print(
        f"[it2-overnight] train={len(train_rows)} val={len(val_rows)} "
        f"roman_attached={sum(1 for r in train_rows if r.get('npi_Roman'))} "
        f"formality_prefix={args.use_formality_prefix}",
        flush=True,
    )
