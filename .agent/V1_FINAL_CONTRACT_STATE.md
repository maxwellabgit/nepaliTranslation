...
public_v1: NO-GO
last_green_commands: mobile jest pendingDeletion-test and accountSummary-test (7 tests, exit 0). Earlier mobile unit suite 75/330 passed at a962b5c. Phase 2 pgTAP BLOCKED: Docker engine is not running and the Supabase CLI is not installed. Upgrade pgTAP was not run.
...
next_action: Start Docker and the Supabase CLI, then run fresh and upgrade pgTAP including 26_phase2_subject_and_sharing.test.sql and the deletion status function. Do not mark Phase 2 or Phase 3 done from unit tests. Public V1 stays NO-GO while the model certificate fails.
